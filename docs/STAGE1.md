...
> **Статус 2026-06-10:** после S11–S17 (НАПП живой sandbox, Android-часть) на ветке `main` сделано:
> мобильные UI-доработки (страница детали авто `VehicleDetailScreen` + `POST /me/vehicles/:id/sync-napp`;
> скрытие нижнего меню на всех внутренних экранах; fade-overlay + поведение «large title» на
> Полисы/Гараж/Профиль через `useCollapsingHeader`).
> **Текущий модуль — S13 Европротокол.** Research готов (`EUROPROTOCOL.md`). Изучен официальный бланк УЗ
> (`assets/europrotocol/`): плоский скан A4×2, заполняется наложением (`pdf-lib`+fontkit). Сделаны
> калибровочная сетка, черновая карта полей (`fieldmap.draft.json`) и тест-рендер; калибровка
> **частичная** (см. `docs/europrotocol/PDF_FILL.md`). **Решение: сперва рисуем 11 экранов (Вариант A),
> затем генерация PDF + доводка координат.** Ждём от пользователя дизайн/раскладку экранов.
