...
>
> **Последнее обновление:** 2026-07-10 — Голос в европротоколе доведён (2 поля + послушать/перезаписать/лимит), плеер голоса в админке, фиксы AI-лога. **⚠️ Блокер:** новый (платный) ключ Gemini даёт 503 на 3.x-моделях + 2.5-flash отключена для новых ключей — надо проверить, что биллинг привязан к проекту ключа (см. статус ниже). До этого (07-09) — реальный ИИ на Gemini.
