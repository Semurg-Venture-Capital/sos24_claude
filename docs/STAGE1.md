...
>
> **Последнее обновление:** 2026-07-09 — **Реальный ИИ на Gemini**: ИИ-триаж + голосовой «Изоҳ» европротокола (speech-to-text + нормализация), центральный `LlmService` с **логом каждого вызова** (`AiUsageLog`) и страницей «AI-лог» в админке. Free tier (без карты). Бэкенд проверен сквозняком; мобилка (запись голоса, `expo-audio`) — код готов, ждёт сборки под телефон. До этого (07-08) — WHOOP + build 10 в TestFlight.
