...
>
> **Последнее обновление:** 2026-07-13 — **ПРОД обновлён** (med24-справочник + ИИ на Gemini включён) · **iOS build 11 в TestFlight**. ⚠️ Важный урок: не деплоить по `:latest` (разбор — `docs/DEVOPS.md` §5). Ранее (07-11) — **med24: клиники + врачи** (пробно 9 клиник + 127 врачей в «Здоровье», описание/часы на странице клиники); справочник API → `docs/integrations/MED24.md`; фильтр-чипы на **нативном iOS 26 Liquid Glass** (капля-морфинг — план на завтра). Ранее сегодня: починен вход (истёкший provisioning dev-client) + переустановлен билд. До этого (07-10) — голос в европротоколе + блокер по ключу Gemini.
...
> - **B — экран:** `HealthWearableScreen` переписан в **табовый** — липкий стеклянный хедер + **`LiquidGlassChips` табы** (Обзор·Восстановление·Сон·Нагрузка·Тренды), карточки на `GlassView` (макс. Liquid Glass), графики на `react-native-svg` (`whoop/WhoopCharts.tsx`, `whoop/WhoopTabs.tsx`).
> - **C — данные:** провайдер расширен (сон +потребность/эффективность/регулярность, сутки +кДж, `fetchWorkouts`+зоны); снапшот +поля; калории (кДж→ккал), зоны пульса за день, список тренировок. Таб «Сон» (получено/потребность), таб «Нагрузка» (калории+зоны+тренировки).
> - **D — локализация UZ ОТЛОЖЕНА** до общей локализации проекта (экраны сейчас на inline-RU, как остальные health-экраны).
> - Real-провайдер (v2-пути) готов, но не проверен на живом ключе WHOOP; на деве — mock.
>
> **Liquid Glass чипы «Здоровья».** Фильтр-чипы «Врачи»/«Клиники» в стиле Telegram (белый контейнер + серая пилюля-капля с растяжением на лету), скелетон загрузки, мгновенное переключение. Липкий сворачивающийся хедер на «Врачи» (WIP, не перенесён на «Клиники»). Детали — [[project-liquid-glass-chips]].

## Где остановились (2026-07-13)

> **Статус 2026-07-13 — ПРОД обновлён (med24 + ИИ на Gemini) · iOS build 11 залит в TestFlight. Ветка `main`.**
>
> **ПРОД выкачен.** Образы API+admin собраны (amd64, `--build-context pnpmstore=...`) и запушены под **уникальным тегом `v20260713-0a23537`** (git-sha). Миграции применены Job-ом на том же теге — накатились **5 новых**: `ai_usage_log`, `euro_voice_remarks`, `euro_desc_voice`, `doctor_contacts`, `partner_region_directory`. Rollout API (2/2) + admin. Проверено: `api.sos24.uz` / `admin.sos24.uz` → 200.
>
> **ИИ включён в проде.** В `sos24-api-secret` залиты `GEMINI_API_KEY` + `LLM_MODEL=gemini-flash-lite-latest` + `TRIAGE_MODE=llm` (было `mock`). Подтверждено на живом поде. Триаж и голосовой ввод теперь работают в проде (платный tier — следить за расходом токенов в админке «AI-лог»).
>
> **med24 в проде.** Импортёр прогнан против прод-БД (port-forward): **9 клиник + 132 врача**, с описаниями и часами. Проверено через `api.sos24.uz/health/clinics`. Полную базу тянем позже (план — `docs/integrations/MED24.md`).
>
> **iOS build 11 → TestFlight (залит).** Номера синхронизированы (app.json/Info.plist/pbxproj = 11), Release-архив собран; в бандле зашит **`https://api.sos24.uz`** (dev-IP отсутствует). Загружен через Xcode Organizer → Distribute (App Store Connect API-ключа на машине нет, загрузка — вручную).
>
> **⚠️ ВАЖНЫЙ ИНЦИДЕНТ — не деплоить по `:latest`.** Push `:latest` прошёл частично, манифест в реестре указывал на **старый** образ → migrate-Job увидел 33 миграции вместо 38 и написал «No pending», новые колонки в прод-БД не появились. `imagePullPolicy: Always` не спасает (тянет `:latest` = старый digest). Вскрылось сверкой digest пода (`imageID`) с `docker inspect RepoDigests`. **Правило: деплоить только по уникальному тегу (git-sha).** Разбор + чек-лист + актуальная ручная процедура — в **`docs/DEVOPS.md` §5**.
>
> **🔧 Починен PDF европротокола в проде (тег `v20260713-2a4a249`).** Симптом: «Получить PDF» → 500, в логах `Failed to launch the browser process … Code: null`. Причина — **Debian-пакет `chromium` (v150) падает по SIGTRAP** на нодах (краш воспроизводится даже из шелла, без puppeteer). НЕ помогло: не-root, `--headless=old`, `--single-process`, `seccomp Unconfined`, `/dev/shm` 256Mi, память 1.5Gi. **Решение:** в Dockerfile вместо apt-chromium — **пиненый Chrome for Testing 131.0.6778.204** (+ зависимости и шрифты); заодно `/dev/shm`=256Mi и лимит памяти 768Mi→1536Mi. Проверено на проде: PDF 565 КБ, 2 стр., 4с. Разбор + правило («версию Chrome пиним явно») — `docs/DEVOPS.md` §5.
>
> **Дальше:** капля-морфинг Liquid Glass чипов (нативно, без пересборки — план в [[project-liquid-glass-chips]]); логотипы клиник / фото врачей; полный импорт базы med24.

## Где остановились (2026-07-11)

> **Статус 2026-07-11 — med24: клиники + врачи в «Здоровье» (пробно, тест на 10); описание/часы клиник; фильтр-чипы на нативном Liquid Glass. Починен вход в приложение. Всё на локальном деве. Ветка `main`.**
>
> **Интеграция med24 (справочник «Здоровье»).** med24 дали официальный доступ (`https://main.med24.uz`, OpenAPI 3.0; отвечает и без токена). **Полный справочник API — `docs/integrations/MED24.md`.** Импортёр **`apps/api/prisma/import-med24.ts`** (`MED24_MODE=clinics|doctors|both`): клиники → `Partner healthDirectory=true` (ключ `med24-clinic-<id>`, область **по координатам** = ближайший из 14 центров РУз, часы «Пн–Сб/09:00–16:00» → `workingHours`, **description**); врачи → `Doctor bookingEnabled=false` («Позвонить» на ресепшн, ключ `med24-doc-<docId>-<clinicId>`, привязка к клинике по `clinic_id`). **⚠️ Грабли:** серверный фильтр по клинике не работает → врачи тянутся пагинацией `GET /api/v1/app/doctors` (per_page=500, ~14 стр.) с локальной фильтрацией; **UA обязателен** (иначе 403); часть клиник отдаёт 500 (напр. 3563). **Пробно залито: 9 клиник + 127 врачей** (Medion Family 46, De Factum Kids 20, B2B 18, ARK 13, …). Видно в «Здоровье»→«Клиники»/«Врачи», в каталог «Партнёры» не попадает. Детали — [[project-med24-import]].
>
> **Страница клиники обогащена.** На детали клиники (`health.service.clinicDetail` + `HealthClinicProfileScreen`) выведены **описание med24** («О клинике»), **часы работы** (форматтер `{mon:{open,close}} → «Пн–Сб · 09:00–16:00»`) и список врачей. У карточек врачей на странице клиники кнопка «Позвонить» убрана (проп `hideAction` в `MedDoctorCard`) — общая «Позвонить» на ресепшн осталась внизу.
>
> **Фильтр-чипы «Врачи»/«Клиники».** (1) Фикс раскладки: список → `flex:1`, строка фильтров → `flexShrink:0`, чип → `alignSelf:'flex-start'` — раньше строка чипов сжималась по высоте при появлении данных. (2) Чипы переведены на **нативный iOS 26 Liquid Glass** — компонент `GlassChip` (`expo-glass-effect` `GlassView`, `isInteractive`, `tintColor` активного; фолбэк expo-blur). **Не доделано:** пользователь хочет «каплю-морфинг» как в Telegram — план на завтра (нативно через `GlassContainer(spacing)` + движущаяся капля, **без пересборки**; `glassEffectID` в JS не проброшен). Детали — [[project-liquid-glass-chips]].
>
> **✅ Вход в приложение починен.** Была цепочка: OTP `6330` → «неверный код», хотя бэкенд отдавал **200 с токенами**. Реальная причина — **истёк provisioning profile dev-client** (→ `expo-secure-store` «No keychain is available» → падал `setSession`; iOS: «приложение больше недоступно»). Лечение: defensive-фикс `apps/mobile/src/lib/secure.ts` (Keychain недоступен → AsyncStorage-fallback) + **пересборка dev-client** (`-allowProvisioningUpdates -allowProvisioningDeviceRegistration`) + **перезагрузка iPhone** (вылечила `installd`, ошибка `0xe8008015` при валидной подписи) → **установлено, вход работает**. Временный отладочный код (`[DEBUG verify-otp]`, `DEBUG:`-строка) — **убран**. Детали — [[project-mobile-devclient]].
>
> **Осталось по «Здоровью»/med24:** капля-морфинг чипов (нативно); логотипы клиник / фото врачей (ссылки vs MinIO); **полный импорт всей базы** (сейчас тест на 10); деплой на прод.

## Где остановились (2026-07-10)

> **Статус 2026-07-10 — Голосовой ввод в европротоколе доведён до продакшн-UX; плеер голоса в админке; фиксы AI-лога. ⚠️ Открытый блокер по ключу Gemini. Всё на локальном деве. Ветка `main`.**
>
> **Голос в европротоколе (шаг 3).** Компонент `VoiceRemarks` доработан: запись → **послушать / перезаписать** (перезапись с подтверждением, не начинает сразу) → «Распознать» (только тогда загрузка+транскрипция). **Лимит перезаписей = 3** (защита токенов). Голосовой ввод добавлен ко **второму** полю — «Описание обстоятельств» (`description`), поля `EuroProtocol += descAudioKey/descRaw`. На iPhone проверено: **и русский, и узбекский** распознаются (напр. описание на узб. «Assalomu alaykum…», замечания на рус.).
>
> **Админка.** Плеер голосовых записей на детали европротокола (`VoiceField`: текст + `<audio>` по presigned-URL + «сырой транскрипт»). AI-лог: починен скролл (обёртка `flex-1 overflow-auto`) + пагинация видна всегда.
>
> **⚠️ Блокер по ключу Gemini (нужно действие заказчика).** Наблюдения: (1) `gemini-2.5-flash` теперь отдаёт **404 «no longer available to new users»** для новых ключей — Google переводит на Gemini 3.x; модель в коде/ENV переключена на **`gemini-flash-latest`**. (2) Новый (по словам заказчика — платный) ключ на **всех 3.x-моделях стабильно 503 «high demand»** даже с ретраями — для реально платного проекта это ненормально, вероятно **биллинг привязан не к проекту ключа** (проект `828510807721`). Что проверить: в aistudio.google.com у проекта ключа тариф должен быть **Paid** (не Free); в Cloud Console к проекту `828510807721` привязан платёжный аккаунт. (3) Старый ключ (free) работает с `gemini-2.5-flash`, но упирается в **429** (суточный лимит). Добавлен авто-ретрай на 503/429 в `LlmService`. Пока блокер не решён — триаж/голос показывают graceful-фолбэк. Детали — [[project-ai-gemini]].
>
> **Прод по-прежнему без ИИ** — вся Gemini-часть только на деве (триаж=mock в проде, голоса нет, ключа в секрете нет). Выкатывать после решения вопроса с ключом.

## Где остановились (2026-07-09)

> **Статус 2026-07-09 — Реальный ИИ на Google Gemini (free tier): триаж + голосовой «Изоҳ» + AI-лог. Всё на локальном деве (прод не трогали). Ветка `main`.**
>
> **Центральный `LlmService` (`apps/api/src/llm/`, @Global).** Все вызовы к Gemini идут через него: `generate/generateJson` (строгий JSON через responseSchema) + `transcribe` (аудио). Каждый вызов пишет **`AiUsageLog`** (feature, model, токены prompt/output/total, latencyMs, ok/error, userId) — это и есть «AI-лог». Конфиг `llm.config` (`GEMINI_API_KEY`, `LLM_MODEL`=gemini-2.5-flash, safety BLOCK_NONE). Модель на free tier поддерживает и текст, и **аудио** (проверено).
>
> **ИИ-триаж на Gemini.** `LlmTriageProvider` (переключение `TRIAGE_MODE=mock|llm`; mock — фолбэк без ключа) через `LlmService`: `triage_ask` (уточняющие вопросы + quickReplies) и `triage_finalize` (вердикт/срочность/специальность). Логируется. Проверено локально.
>
> **Голосовой «Изоҳ» европротокола (speech-to-text + нормализация).** `POST /europrotocol/remarks/transcribe` (stateless): аудио из MinIO → Gemini → `{transcript, normalized, language}`, авто-язык RU/UZ. Нормализация **КОНСЕРВАТИВНАЯ** (юр. документ: чиним орфографию/пунктуацию, факты не меняем). `EuroProtocol += remarksAudioKey` (голос-первоисточник) + `remarksRaw` (сырой транскрипт); сохраняются при submit. `files` — разрешены audio-типы (kind=audio). Мобилка: компонент **`VoiceRemarks`** в шаге 3 (`expo-audio`: запись→загрузка→«распознаём…»→редактируемый текст). Проверено сквозняком на RU-аудио.
>
> **Админка «AI-лог».** `GET /admin/ai-usage` (список+сводка: всего запросов/токенов, по фичам) → страница `/ai-usage` (карточки, бары по функциям, таблица, фильтр). Пункт в сайдбаре.
>
> **Ключи Gemini:** локально в `apps/api/.env` (`TRIAGE_MODE=llm`, `GEMINI_API_KEY`, `LLM_MODEL`); в прод НЕ клали. secret.example — плейсхолдеры. Free tier (лимиты по RPM/сутки; при превышении — graceful-фолбэк). Карту заказчик подключит позже — код не изменится.
>
> **⏳ Осталось:** (1) **собрать dev-client под телефон + поставить** (был недоступен) → проверить голос вживую, **включая узбекский** на реальной записи; на iPhone аудио `.m4a` (шлём `audio/mp4`) — убедиться, что Gemini принимает, иначе переключить пресет записи на WAV. (2) Выкатить в прод (GEMINI_API_KEY в секрет + `TRIAGE_MODE=llm` + пересборка образа API) — когда решим. (3) dev-client всё ещё смотрит в прод (Metro с `EXPO_PUBLIC_API_URL`); для локального теста триажа/голоса — перезапустить Metro без этой переменной.
