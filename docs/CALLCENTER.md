...
## 11. Исходящие звонки оператора (dial-out) — 2026-07-23

> Цель: оператор звонит **из страницы колл-центра** (`admin.sos24.uz` → Колл-центр) — как перезвон по журналу/карточке входящего, так и ручной набор номера.

### 11.1 Архитектура: звонок инициирует браузер (client-originated)

Вызов уходит **из браузера через SIP.js `Inviter`** на уже зарегистрированный UA оператора (ext 102/105) — тот же WSS + coturn/TURN путь, что и на входящих. Сервер (ARI Originate) **не** используется: оператор уже в браузере, лишнее плечо не нужно.

```
браузер (ext 102, WSS) → Asterisk .30 → [outbound route] → транк на .29 → провайдер → PSTN
                          ↑ медиа через coturn (тот же, что на входящих)
```

### 11.2 Наша сторона — ✅ ГОТОВО и РАБОТАЕТ (dev, 2026-07-23, двусторонний звук подтверждён)

- `apps/admin/src/lib/softphone.ts` — метод `Softphone.call(number)`: нормализует номер, строит `sip:<num>@<domain>`, создаёт `Inviter`, переиспользует ту же медиа-машинерию, что и на входящих. Состояние `outgoing`.
- `SoftphoneBar` — поле ввода номера + раскрывающаяся клавиатура (1-9 * 0 #) + «Позвонить»; экран исходящего вызова («Вызов… <номер>» + «Отменить»); проброс `dial()`/состояния наверх.
- `call-center/page.tsx` — кнопки **«Перезвонить»** в строках журнала и карточках входящих (гаснут, когда софтфон занят/не зарегистрирован).

**Две грабли, которые чинили при отладке (обе в `call()`):**
1. **Пустой SDP → Asterisk `488 Not Acceptable Here`.** Определяли микрофон через `enumerateDevices()`, но до выдачи прав браузер отдаёт устройства с пустым `kind` → ложно «нет микрофона» → `constraints{audio:false}` → SIP.js слал оффер **без `m=audio`** → 488 (на входящих не всплывало: там m-строку даёт Asterisk). **Фикс: исходящий ВСЕГДА `constraints{audio:true}`.**
2. **Формат номера — провайдер принимает НАЦИОНАЛЬНЫЙ формат без `998`.** `998993286330` не проходил, `993286330` — проходил. **Фикс: срезаем ведущие `998` при длине >9.**

### 11.3 На Asterisk .30 — исходящий маршрут УЖЕ настроен, менять НЕ НУЖНО

Диагностикой по SSH (`ssh -i ~/.ssh/sos24_nodes root@10.10.10.30`) выяснилось, что всё уже на месте — отдельного ТЗ сисадмину не потребовалось:
- **Маршрут `outrt-1`** («operator-out»), паттерны `_X.` / `_XXXXXXXXX` / `_XXXXXXXXXXXX` — ловят любой внешний номер.
- **Транк `OUT_1_SUFFIX=@2050855`** → набор `PJSIP/<номер>@2050855` → провайдер `10.10.0.3` (endpoint `2050855` зарегистрирован, Avail).
- **CallerID уже = 2050855** (`TRUNKCIDOVERRIDE=2050855`, OUTCID `712050855`) — подтверждён в логе плеча к провайдеру: `From: <sip:2050855@10.10.10.30>` → `200 OK`.
- **Запись исходящих включена** (`sub-record-check,s,1(out,${EXTEN},force)`) → аплоадер заберёт (`REC_S3_*`).
- Ext 102 (context `from-internal`) доходит до `outrt-1` штатно (проверено `dialplan show <num>@from-internal`).

> ⚠️ AMI-юзер `sos24cc` НЕ имеет права `command` — CLI-диагностика (`pjsip show`, `dialplan show`) только по **SSH** (`asterisk -rx`). Отладка 488: `pjsip set logger on` → тест-звонок → grep `/var/log/asterisk/full` по номеру/Call-ID → `pjsip set logger off`. Бокс общий с сотрудниками — держать логгер коротко, чистить за собой.

### 11.4 ⏳ Наша сторона — фаза 2

- **Журналирование исходящих:** ARI-обозреватель на .30 видит канал `PJSIP/102-…`; надо классифицировать `direction=OUTBOUND` и создать `Call`-строку (эмпирически по реальным событиям, как делали для входящих). Сейчас исходящий в журнале может не появляться.
- **DTMF в разговоре** (навигация по IVR) — послать тоны через активную сессию; сейчас клавиатура только для набора до звонка.
- Код исходящих НЕ закоммичен и НЕ в проде.
