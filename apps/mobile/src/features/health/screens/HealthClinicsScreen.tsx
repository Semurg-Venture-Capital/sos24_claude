  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: tokens.pageBg }} edges={['top']}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 14, paddingHorizontal: 24, paddingTop: 8, paddingBottom: 12 }}>
        ...
        <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 18, color: tokens.ink }}>{t('healthCard.clinics.title')}</Text>
      </View>
      ...
      <View style={{ paddingHorizontal: 24 }}>
        <View style={{ height: 52, paddingHorizontal: 18, borderRadius: 999, backgroundColor: tokens.glass, borderWidth: 1, borderColor: tokens.hairline, flexDirection: 'row', alignItems: 'center' }}>
          <TextInput value={q} onChangeText={setQ} placeholder={t('healthCard.clinics.searchPh')} placeholderTextColor={tokens.inkMuted} style={{ flex: 1, fontFamily: 'Manrope_400Regular', fontSize: 15, color: tokens.ink }} />
        </View>
      </View>

      <LiquidGlassChips
        items={[{ key: ALL, label: t('healthCard.clinics.allRegions') }, ...regions.map((r) => ({ key: r, label: r }))]}
    ...
    <Pressable onPress={onPress} style={({ pressed }) => [{ borderRadius: 22, padding: 16, gap: 12, opacity: pressed ? 0.9 : 1 }, medGlass]}>
      <View style={{ gap: 4 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 2 }}>
          ...
          {c.doctorCount > 0 ? (
            <Text style={{ fontFamily: 'Manrope_400Regular', fontSize: 13, color: tokens.inkMuted }}>{t('healthCard.clinics.doctorsCount', { count: c.doctorCount })}</Text>
          ) : null}
      ...
      >
        ...
        <Text style={{ fontFamily: 'Manrope_600SemiBold', fontSize: 14, color: '#fff' }}>{t('healthCard.common.call')}</Text>
      </Pressable>
    </Pressable>
  );
}
