...
>
> **Последнее обновление:** 2026-05-21 (вечер)

---

## Где остановились

**Последние завершённые задачи:** S5 Partners, ProfileEdit fix, расширение хранилища MyID.
...
### Вариант D — i18n cleanup + lint + тесты (~2 дня) ← СЛЕДУЮЩИЙ
Вынести русские строки C7–C12 в `packages/i18n-strings`, настроить lint в turbo, написать первые integration-тесты на auth+/me/policies.

### Вариант C — Этап C13 (~3-4 дня, нужен дизайнер)
Дизайны M9/M10/M11/M12/M13/M16 от дизайнера → переносим. Без дизайна — риск переделок.

**Рекомендованный порядок:** D → C13 → Admin Phase 2.
...
| `apps/api/` | NestJS бэк (10 модулей, Prisma 6, seed) |
| `apps/api/src/{auth,users,vehicles,napp,documents,drivers,policies,promo,cards,wallet,payments,admin,partners}` | модули |
| `apps/api/prisma/{schema.prisma,migrations,seed.ts}` | БД |
| `apps/mobile/` | Expo + RN |
| `apps/mobile/src/api/` | API-слой: types, 10 domain-файлов с хуками (+ partners.ts) |
