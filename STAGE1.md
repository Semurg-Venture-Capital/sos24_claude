...
**Последние завершённые задачи:** S10 Финансы (Finance-экран в Профиле, mock-баланс карт, Payme/Click init, set-default карты, история операций). Перед этим: S9 Аджастер (полный модуль). Исследование Европротокола завершено, реализация отложена до решения руководства.
...
Когда придут ключи от MyID:
1. `MYID_MOCK=false` в `apps/api/.env`
2. Установить `myid-rn-sdk` (закрытый GitLab `myid-public-code/myid-rn-sdk`) + написать Expo Config Plugin
3. Заменить TODO в `MyIdOnboardingScreen.startMyId()` на `MyIdClient.start(sessionId)`
...
### S10 — Европротокол ← СЛЕДУЮЩИЙ (нужен дизайн + решение по варианту)
...
**Рекомендованный порядок:** S10 Европротокол → Admin Phase 2 → D (i18n/тесты).
...
| `apps/api/` | NestJS бэк (11 модулей, Prisma 6, seed) |
| `apps/api/src/{auth,users,vehicles,napp,documents,drivers,policies,promo,cards,wallet,payments,admin,partners,adjuster}` | модули |
| `apps/api/prisma/{schema.prisma,migrations,seed.ts}` | БД |
| `apps/mobile/` | Expo + RN |
| `apps/mobile/src/api/` | API-слой: types, 11 domain-файлов с хуками (+ partners.ts, adjuster.ts) |
| `apps/mobile/src/components/ui/` | UI-кит ~40 примитивов + `AdjusterActiveBanner` |
| `apps/mobile/src/features/{auth,policies,purchase,profile,garage,main,adjuster}/` | бизнес-фичи |
| `apps/mobile/src/navigation/*Navigator.tsx` | стеки |
| `apps/admin/` | Next.js 15 — веб-админка страховой |
| `apps/admin/src/app/(dashboard)/` | Dashboard, Users, Policies, Adjuster страницы |
